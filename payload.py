...
class QuadcopterLQR:
    ...
    def create_drone(self):
        ...
        m_payload = 0.25 * self.m
        L_rope = 0.25

        # Create payload body (sphere)
        payload_col = p.createCollisionShape(p.GEOM_SPHERE, radius=0.05)
        payload_vis = p.createVisualShape(p.GEOM_SPHERE, radius=0.05, rgbaColor=[0, 0, 0, 1])

        # Spawn payload below the drone
        base_pos, base_quat = p.getBasePositionAndOrientation(drone_id)
        payload_start = [base_pos[0], base_pos[1], base_pos[2] - L_rope]

        payload_id = p.createMultiBody(
            baseMass=m_payload,
            baseCollisionShapeIndex=payload_col,
            baseVisualShapeIndex=payload_vis,
            basePosition=payload_start
        )

        # Make a "rope" as a ball joint at the drone, with the payload COM offset by L_rope
        cid = p.createConstraint(
            ...
            jointAxis=[0, 0, 0],
            parentFramePosition=[0, 0, 0],       # attach at drone COM (you can offset if you want)
            childFramePosition=[0, 0, L_rope]    # payload COM is L_rope below the joint
        )

        # Optional: damp payload to keep sim sane
        p.changeDynamics(payload_id, -1, linearDamping=0.01, angularDamping=0.01)
    ...
    def plot_errors(self):
        ...
        fig.suptitle(f'{controller_name} Controller - Tracking Error with Added Payload (No Wind)', 
